...
// Galerisi olan hizmetlerden proje havuzu oluştur
const categories = [
  { key: "all", label: "Tümü" },
  ...services.filter((s) => s.gallery.length > 0).map((s) => ({ key: s.slug, label: s.title })),
];

const allItems = services
  .filter((s) => s.gallery.length > 0)
  .flatMap((s) => s.gallery.map((src) => ({ src, cat: s.slug, catLabel: s.title })));
...
export default function ProjectsGallery() {
  ...
  const items = useMemo(
    () => (filter === "all" ? allItems : allItems.filter((i) => i.cat === filter)),
    [filter]
  );
  ...
  return (
    <div>
      ...
      <div className="pgrid">
        {items.map((it, i) => (
          <button key={it.src + i} className="pcell" onClick={() => setLb(i)} aria-label={`${it.catLabel} görsel`}>
            <Image src={it.src} alt={it.catLabel} fill sizes="(max-width:700px) 50vw, 33vw" className="pimg" />
            <span className="pcat">{it.catLabel}</span>
          </button>
        ))}
      </div>
